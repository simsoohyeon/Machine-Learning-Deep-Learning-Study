...
# 각 미니배치에 대해 IncrementalPCA의 partial_fit
메서드를 호출하여 모델을 점진적으로 학습

X_reduced = inc_pca.transform(X_train)  # 전체 훈련 데이터셋을 변환하여 축소된 데이터셋

점진적 PCA는 메모리 사용을 최적화하면서도 대규모 데이터셋에 PCA를 적용할 수 있
```
### 8.5 지역 선형 임베딩 (1)
```
- 지역 선형 임베딩 locally linear enbedding LLE
비선형 차원 축소: 비선형 구조를 유지하면서 고차원 데이터를 저차원으로 변환
pca나 랜덤 투영과는 달리 투영에 의존하지 않는 매니폴드 학습: 데이터의 복잡한 구조 보존
잡음이 많지 않은 경우 매니폴드를 펼치는데 좋음: 복잡한 매니폴드 구조 학습

from sklearn.datasets import make_swiss_roll
# make_swiss 함수를 사이킷런 데이터셋 모듈에서 임포트
스위스 룰 데이터셋을 생성하는데 사용, 스위스롤은 고차원 공간에서 꼬인 매니폴드 형상을 가지는 데이터셋

from sklearn.manifold import LocallyLinearEmbedding
# LocallyLinearEmbedding 클래스를 사이킷런의 매니폴드 모듈에서 임포트
지역 선형 임베딩 알고리즘을 사용하여 비선형 차원 축소를 수행하는데 사용

X_swiss, t = make_swiss_roll(n_samples=1000, noise=0.2, random_state=42)
# make_swiss_roll 함수 사용하여 스위스 롤 데이터셋 생성
n_samples=1000 1000개의 샘플 생성, noise=0.2 데이터에 0.2 잡음 추가
random_state=42 난수 생성을 위한 시드 값을 42로 설정하여 재현 가능한 결과를 얻음
X_swiss에는 스위스 롤 데이터의 좌표가 저장되고
t에는 각 데이터 포인트 위치를 나타내는 타겟 변수 저장

lle = LocallyLinearEmbedding(n_components=2, n_neighbors=10, random_state=42)
# LocallyLinearEmbedding 객체 생성
n_components=2 최종적으로 2차원으로 차원 축소
n_neighbors=10 각 포인트의 이웃으로 10개 사용, 지역 선형 근사 만들 때 사용

X_unrolled = lle.fit_transform(X_swiss)
# fit_transfrom 메소드 사용하여 LLE 알고리즘 적용
X_swiss 데이터셋을 입력으로 받아 학습을 수행하고 차원 축소
X_unrolled 에는 2차원으로 축소된 데이터 셋이 저장, 이는 원래는 스위스 롤 형태를 펼친 결과
